% solution of the ODE d^2(EI*(d^2w/dx^2))/dx^2 = f0 for 0<x<L
% with boundary conditions W(0) = 0, w(L) = 0, EI*d^2w/dx^2 (at x=0) = 0
% and EI*d^2w/dx^2 (at x=L) = 0
...
phi_array = createPhi(N, x, L);
f = createF(N, phi_array, phi_0, x, L, E, I, f0) ;
...
function [f] = createF(N, phi_array, phi_0, x, L, E, I, f0)
    f = zeros(N, 1) ;
    for i=1:N
        f(i) = linearFunctional(phi_array, phi_0, i, x, L, E, I, f0) ;
    end
end
...
function [f_i] = linearFunctional(phi_array, phi_0, i, x, L, E, I, f0)
    phi_i = phi_array(i) ;
    didx = diff(phi_i, x) ;
    d0dx = diff(phi_0, x) ;

    f_i = int(phi_i*f0, 0, L) - int(E*I*didx*d0dx, 0, L) ;
end
